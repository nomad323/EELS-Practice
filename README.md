...
- **选择 / 滚轮微调**：正常模式下 **↑↓ 选择当前页参数**，不改变系数；页首/页尾停住，不跳到隐藏页。单击行或行末 `↔` 也只选择。**双击 `↔` 或按 Enter** 才开始调整；双击数值框不启用。选择行和活动行有不同高亮。
  ...
  - **滚轮或 ←/→ 改系数**：调节模式下，键盘 **← 减少一个当前步长，→ 增加一个当前步长**；不切换参数、不结束会话。长按按系统重复按键逐步调整；带 Ctrl/Meta/Alt/Shift 的左右键不改系数。与滚轮共用步长、限幅及确认/撤销快照。鼠标在光斑、其他参数或步长框上都只调活动项；上滚增加、下滚减少，每个有效垂直事件一步，系数仍限制在 ±120。调节中不翻页、不触发数值框原生滚动；水平及 Ctrl/Meta+滚轮也被拦截，但不改系数。
